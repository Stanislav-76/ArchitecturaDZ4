        ...
        try {
            ...
            if (mobileApp.buyTicket("11000000221")){
                ...
                if (busStation.checkTicket(tickets.stream().findFirst().get().getQrcode())){
                    ...
                    System.out.println("Клинет не смог пройти в автобус");
                }
...
class MobileApp{
    ...
    public MobileApp(TicketProvider ticketProvider, CustomerProvider customerProvider) throws RuntimeException {
...
class TicketProvider{
    ...
    public Collection<Ticket> searchTicket(int clientId, Date date){
        ...
        for (Ticket ticket: database.getTickets()) {
            if (ticket.getCustomerId() == clientId && ticket.getDate().toString().equals(date.toString()))  //обход ticket.getDate().equals(date)
                tickets.add(ticket);
        }
        return tickets;
    ...
    public boolean buyTicket(int clientId, String cardNo){
        ...
        if(paymentProvider.buyTicket(orderId,  cardNo, amount)){ // Если платеж прошел в билет добавляем покупателя, дату, qrcode
            for(Ticket ticket: database.getTickets()){
    ...
    public boolean checkTicket(String qrcode){
        for (Ticket ticket: database.getTickets()) {
            if (ticket.isEnable() && ticket.getQrcode().equals(qrcode)){
...
class CustomerProvider{
    ...
    private void checkPassword (String login, String password) throws RuntimeException {
        // ПРЕДУСЛОВИЕ
        if (login.length() < 3) throw new RuntimeException("Логин неверный");
        if (password.length() == 0) throw new RuntimeException("Пароль не может быть пустым");
        // TODO сравнение логина и пароля с базой
        boolean loginCheck = true;
        boolean passwordCheck = true;
        // ПОСТУСЛОВИЕ
        if (loginCheck == false || passwordCheck == false) throw new RuntimeException("Логин или пароль введен неверно");
    }
